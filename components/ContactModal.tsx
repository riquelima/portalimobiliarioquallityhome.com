...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose, owner, propertyTitle, onStartChat }) => {
  ...
  const whatsappLink = owner?.phone ? `https://wa.me/${owner.phone}?text=${encodeURIComponent(whatsappMessage)}` : '#';
  ...
  return (
    ...
    >
      <div 
        ...
        className={`relative bg-white rounded-lg shadow-xl w-11/12 max-w-sm p-6 sm:p-8 m-4 transform transition-all duration-300 ease-out ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
      >
        {owner && (
          <>
            <button 
